...
import static DAO.mysql.generatedclasses.tables.Categoria.CATEGORIA;
...
public class PalavraDAO extends DAO<Palavra>{
    ...
    public boolean novo(Palavra novo) {
        PalavraRecord p = null;
        if(!AudioDAO.getInstance().novo(novo.getAudio()) || !ImagemDAO.getInstance().novo(novo.getImagem())){
            return (false);
        }

        if(novo.getVideo() != null){
            VideoDAO.getInstance().novo(novo.getVideo());
            p = GerenciadorBD.getContext().insertInto(PALAVRA)
